function varargout = edgesEvalDir_crack( varargin )
...
ids=dir(fullfile(gtDir,'*.png')); 
ids={ids.name}; 
n=length(ids);
idsRes=dir(fullfile(resDir,'*.png')); 
idsRes={idsRes.name}; 
n=length(idsRes);
...
%writeRes(evalDir,'eval_bdry_thr.txt',[T_orig R_orig P_orig F_orig]);
